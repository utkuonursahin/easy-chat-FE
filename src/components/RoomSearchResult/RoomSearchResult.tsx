...
const RoomSearchResult = () => {
    ...
    const [roomSearchId, setRoomSearchId] = useAtom(findRoomSearchAtom)
    const httpClient = useHttp()

    const onJoinRoom = async () => {
        try{
            const {data: chatRoom, statusCode} = await httpClient.post<ChatRoomDto>(`http://localhost:8080/api/chat-rooms/join/${roomSearchId}`)
